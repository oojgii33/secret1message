function charTo6bit(ch) {
  ...
  // Mongolian Cyrillic: 37-63 (Mapping most common ones to fit in 6 bits)
  const mn = "АБВГДЕЁЖЗИЙКЛМНОӨПРСТУҮФХЦЧШЩЪЫЬЭЮЯ";
  const idx = mn.indexOf(ch);
  if (idx !== -1) return 37 + (idx % 27); // Simple wrap for now to stay in 6 bits
...
export function buildEncodedCanvas(img, text) {
  ...
  // 1. Store length in Green channel LSB (first 18 pixels)
  const lenBits = len.toString(2).padStart(18, "0");
  for (let i = 0; i < 18 && i < w * h; i++) {
    const di = i * 4 + 1; // Green channel
    d[di] = (d[di] & 0xfe) | parseInt(lenBits[i]);
  }

  // 2. Store message in Red channel LSB (starting from pixel 0)
  const encPix = groups.length * 6;
  for (let pi = 0; pi < Math.min(encPix, w * h); pi++) {
    const di = pi * 4; // Red channel
    const origR = d[di];
...
export function decodeFromCanvas(img) {
  ...
  // 1. Read length from Green channel LSB (first 18 bits)
  let lenBits = "";
  if (w * h < 18) return { text: "", groups: [] };
  for (let i = 0; i < 18; i++) {
    lenBits += String(d[i * 4 + 1] & 1);
  }
  const len = parseInt(lenBits, 2);
  const safeLen = Math.min(len, Math.floor((w * h) / 6), 10000);

  let text = "";
  const allGroups = [];

  // 2. Read message groups from Red channel (starting from pixel 0)
  for (let gi = 0; gi < safeLen; gi++) {
    let bits = "";
    const piStart = gi * 6;
    for (let bi = 0; bi < 6; bi++) {
      bits += String(d[(piStart + bi) * 4] & 1);
    }
    const val = parseInt(bits, 2);
    const ch = bit6ToChar(val);
    allGroups.push({ ch, val, bits, isMetadata: false });
    text += ch;
  }

  return { text, groups: allGroups };
}
...
export function renderPreview(
  ...
  const encPix = groups.length * 6;
  const MAX_CANVAS_SIZE = Math.min(16384, Math.max(4000, imgWidth * 4, imgHeight * 4));
  ...
  if (showDetails) {
    ...
    for (let pi = 0; pi < encPix; pi++) {
      ...
      ctx.fillStyle = lsb === 1 ? gc + "0.45)" : gc + "0.08)";
      ...
      if (showText) {
        ...
        if (lsb === 1) {
          ctx.fillStyle = lum > 128 ? "#ff0000" : "#ff6666"; // Make +1 pop
